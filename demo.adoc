...
To convert the Quarkus application to reactive, change the SmallRye JWT dependency to Quarkus OIDC in `build.gradle`:
...
Change the `application.properties` to use OIDC:
...
And update `HelloResource.java` to read the name from the JWT:
...
https://docs.spring.io/spring-framework/reference/web/webflux.html[Spring WebFlux] is the reactive web framework for Spring Boot.
